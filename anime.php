...
<body>
	...
	if($notes->rowCount() > 0) {
		...
		$reqnote = $bdd->prepare('SELECT id_note FROM note_membre WHERE ID_anime = ? AND Description NOT LIKE ""');
		$reqnote->execute(array($id_anime));

		if($reqnote->rowCount() == 0) {
			echo $note_final."/<strong>10</strong> (".$nombre_notes." notes</a>)";
